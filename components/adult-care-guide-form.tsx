...
import { CheckCircle, AlertCircle, ArrowLeft, X, Heart, User, FileText, MessageCircle, Phone, MoreHorizontal } from "lucide-react"
import { submitClientApplication } from "@/app/actions/simple-forms"

interface AdultCareGuideFormProps {
  onClose?: () => void
  onBack?: () => void
}

export function AdultCareGuideForm({ onClose }: AdultCareGuideFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formDataObj.append("careType", "adult_care_guide")
      ...
      // Add guide services
      formData.guideServices.forEach((service) => {
        formDataObj.append("guideServices", service)
      })
      ...
      if (result.success) {
        setTimeout(() => {
          if (onClose) {
            ...
            window.location.href = "/care/adult-care-guide?submitted=true"
          }
        }, 2000)
      }
    } catch (error) {
  ...
  const WhoNeedsCareStep = () => (
    <div className="min-h-[600px] p-6 space-y-8">
      ...
      <div className="text-center space-y-8">
        <h1 className="text-2xl md:text-3xl font-semibold text-gray-800">Who needs care guidance?</h1>
  ...
  // Step 2: What kinds of guidance services are needed?
  const GuideServicesStep = () => (
    <div className="min-h-[600px] p-6 space-y-8">
      <div className="flex items-center justify-between">
      ...
      <div className="text-center space-y-8">
        <h1 className="text-2xl md:text-3xl font-semibold text-gray-800">What guidance services do you need?</h1>
        
        <div className="max-w-md mx-auto space-y-3">
          {[
            { id: "care_planning", label: "Care planning guidance", icon: FileText },
            { id: "consultation", label: "Care consultation", icon: MessageCircle },
            { id: "phone_support", label: "Phone support", icon: Phone },
            { id: "other", label: "Other", icon: MoreHorizontal },
          ].map((service, index) => {
            const Icon = service.icon
            const isSelected = formData.guideServices.includes(service.id)
            ...
            return (
              <motion.button
                ...
                onClick={() => {
                  const updated = isSelected
                    ? formData.guideServices.filter(type => type !== service.id)
                    : [...formData.guideServices, service.id]
                  setFormData({ ...formData, guideServices: updated })
                }}
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <span className="text-base font-medium text-gray-800">{service.label}</span>
                </div>
        ...
        <div className="pt-4">
          <Button
            onClick={handleNext}
            disabled={formData.guideServices.length === 0}
    ...
    <div className="min-h-[600px] p-6 space-y-8">
      ...
      <div className="max-w-md mx-auto space-y-6">
        <h1 className="text-2xl md:text-3xl font-semibold text-gray-800 text-center">
          {formData.whoNeedsCare === "parent_loved_one" ? "Get Guidance for a Loved One" : "Get Care Guidance"}
        </h1>
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-gray-500 text-center">
            By clicking "Request Care Guidance", you agree to our Terms of Service and Privacy Policy.
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="w-full max-w-4xl mx-auto">
      <AnimatePresence mode="wait" custom={direction}>
        ...
        >
          ...
          {currentStep === 2 && <GuideServicesStep />}
