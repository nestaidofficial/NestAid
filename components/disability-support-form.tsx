...
import { CheckCircle, AlertCircle, ArrowLeft, X, Heart, User, Accessibility, Users, Activity, MoreHorizontal } from "lucide-react"
import { submitClientApplication } from "@/app/actions/simple-forms"

interface DisabilitySupportFormProps {
  onClose?: () => void
  onBack?: () => void
}

export function DisabilitySupportForm({ onClose }: DisabilitySupportFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formDataObj.append("careType", "disability_support")
      ...
      formData.supportTypes.forEach((type) => {
        formDataObj.append("supportTypes", type)
      })
      ...
      if (result.success) {
        setTimeout(() => {
          if (onClose) {
            ...
            window.location.href = "/care/disability-support?submitted=true"
          }
        }, 2000)
      }
    } catch (error) {
  ...
  const WhoNeedsCareStep = () => (
    <div className="min-h-[600px] p-6 space-y-8">
      ...
      <div className="text-center space-y-8">
        <h1 className="text-2xl md:text-3xl font-semibold text-gray-800">Who needs support?</h1>
  ...
  // Step 2: What kinds of disability support are needed?
  const SupportTypesStep = () => (
    <div className="min-h-[600px] p-6 space-y-8">
      <div className="flex items-center justify-between">
      ...
      <div className="text-center space-y-8">
        <h1 className="text-2xl md:text-3xl font-semibold text-gray-800">What kinds of support are needed?</h1>
        
        <div className="max-w-md mx-auto space-y-3">
          {[
            { id: "accessibility_support", label: "Accessibility support", icon: Accessibility },
            { id: "personal_care", label: "Personal care assistance", icon: Users },
            { id: "daily_activities", label: "Daily activities support", icon: Activity },
            { id: "other", label: "Other", icon: MoreHorizontal },
          ].map((support, index) => {
            const Icon = support.icon
            const isSelected = formData.supportTypes.includes(support.id)
            ...
            return (
              <motion.button
                ...
                onClick={() => {
                  const updated = isSelected
                    ? formData.supportTypes.filter(type => type !== support.id)
                    : [...formData.supportTypes, support.id]
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <span className="text-base font-medium text-gray-800">{support.label}</span>
                </div>
        ...
        <div className="pt-4">
          <Button
            onClick={handleNext}
            disabled={formData.supportTypes.length === 0}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-base font-medium text-gray-700">
              Postal code where support is needed
            </Label>
            <Input
          ...
          <p className="text-xs text-gray-500 text-center">
            By clicking "Request Support Consultation", you agree to our Terms of Service and Privacy Policy.
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="w-full max-w-4xl mx-auto">
      <AnimatePresence mode="wait" custom={direction}>
        ...
        >
          ...
          {currentStep === 2 && <SupportTypesStep />}
