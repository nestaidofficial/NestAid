...
import { CheckCircle, AlertCircle, ArrowLeft, X, Heart, User, Brain, Users, Activity, MoreHorizontal } from "lucide-react"
import { submitClientApplication } from "@/app/actions/simple-forms"

interface SpecialNeedsCareFormProps {
  onClose?: () => void
  onBack?: () => void
  inModal?: boolean
}

export function SpecialNeedsCareForm({ onClose, inModal = false, onBack }: SpecialNeedsCareFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formDataObj.append("careType", "special_needs_care")
      ...
      // Add special needs types
      formData.specialNeedsTypes.forEach((type) => {
        formDataObj.append("specialNeedsTypes", type)
      })
      ...
      if (result.success) {
        setTimeout(() => {
          if (onClose) {
            ...
            window.location.href = "/care/special-needs-care?submitted=true"
          }
        }, 2000)
      }
    } catch (error) {
  ...
  // Step 2: What kinds of special needs support are needed?
  const SpecialNeedsTypesStep = () => (
    <div className="min-h-[600px] p-6 space-y-8">
      <div className="flex items-center justify-between">
      ...
      <div className="text-center space-y-8">
        ...
        <div className="max-w-md mx-auto space-y-3">
          {[
            { id: "cognitive_support", label: "Cognitive support", icon: Brain },
            { id: "behavioral_support", label: "Behavioral support", icon: Users },
            { id: "physical_assistance", label: "Physical assistance", icon: Activity },
              ...
              <button
                ...
                onClick={() => {
                  const updated = isSelected
                    ? formData.specialNeedsTypes.filter(type => type !== support.id)
        ...
        <div className="pt-4">
          <Button
            onClick={handleNext}
            disabled={formData.specialNeedsTypes.length === 0}
    ...
    <div className="w-full max-w-4xl mx-auto">
      <AnimatePresence mode="wait" custom={direction}>
        ...
        >
          ...
          {currentStep === 2 && <SpecialNeedsTypesStep />}
