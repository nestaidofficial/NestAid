    ...
    <Dialog open={isOpen} onOpenChange={onClose} className="z-[99999]">
      <DialogContent className="w-full h-full max-w-none max-h-none m-0 p-0 bg-[#FCF5EB] md:max-w-4xl md:max-h-[90vh] md:m-auto md:rounded-3xl overflow-y-auto z-[99999] [&>button]:absolute [&>button]:right-6 [&>button]:text-[#1E4D47]">
        <div className="p-6 md:p-8 lg:p-10">
          ...
          {/* Job Overview */}
          <div className="bg-[#3F736D] rounded-2xl p-6 md:p-8 shadow-sm">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-[#D9FB74] rounded-full flex items-center justify-center shadow-sm">
                  <MapPin className="w-6 h-6 text-[#1E4D47]" />
                </div>
                <div>
                  <p className={`${inter.className} font-semibold text-white mb-1`}>Location</p>
                  <p className={`${inter.className} text-white/80`}>{job.city}, {job.state} {job.zipcode}</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-[#D9FB74] rounded-full flex items-center justify-center shadow-sm">
                  <Calendar className="w-6 h-6 text-[#1E4D47]" />
                </div>
                <div>
                  <p className={`${inter.className} font-semibold text-white mb-1`}>Posted</p>
                  <p className={`${inter.className} text-white/80`}>{formatDate(job.created_at)}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <h3 className={`${playfair.className} text-2xl md:text-3xl font-bold text-[#1E4D47] flex items-center gap-3`}>
              <Heart className="w-6 h-6 text-[#3F736D]" />
              About This Position
            </h3>
            <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-100 shadow-sm">
              <p className={`${inter.className} text-[#1E4D47]/70 text-base md:text-lg leading-relaxed mb-8`}>
                ...
                <div>
                  ...
                  <ul className={`${inter.className} space-y-3 ml-2`}>
                    <li className="flex items-start gap-3">
                      <div className="w-2 h-2 bg-[#3F736D] rounded-full mt-2 flex-shrink-0"></div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-8 border-t border-gray-200">
            <Button
              ...
              variant="outline"
              size="lg"
              className={`${inter.className} flex-1 border-2 border-[#1E4D47] text-[#1E4D47] hover:bg-[#D9FB74]/20 hover:border-[#1E4D47] font-semibold rounded-full py-4 md:py-3 text-base transition-all duration-300`}
            >
              Close
            </Button>
            <Button
              onClick={() => onApply(job)}
              size="lg"
              className={`${inter.className} flex-1 bg-[#1E4D47] hover:bg-[#163D37] text-white font-semibold rounded-full shadow-md hover:shadow-lg transition-all duration-300 py-4 md:py-3 text-base`}
            >
              Apply Now
            </Button>
          </div>
